...
// fill it in directly: the "Submit Guess to GTB" link carries the prediction in the URL fragment
// (#bg=..., never sent to their server), and the "Fill GTB Form" bookmarklet, clicked while on
// the entry form, reads it back and sets the dropdowns. The user then checks and sends the entry.
...
export function fillGtbForm(doc, hash, notify) {
  ...
  if (!m) {
    notify('No Sumo Ranker picks found. Open this page with the "Submit Guess to GTB" button on Sumo Ranker, then click this bookmark again.');
    return null;
  }
  let picks;
